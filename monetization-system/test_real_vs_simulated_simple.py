...
class FraudComparisonAnalyzer:
    ...
    def simulate_transaction_with_balance_holder(self, amount: Decimal, currency: str = 'BTC') -> dict:
        ...
        initial_balance = self.balance_holder.get_total_balance(currency, include_inflated=True, include_fake=True)
        print(f"Initial Balance: {initial_balance}")

        # Attempt withdrawal (simulate by trying to deduct)
        transaction_id = f"test_withdrawal_{int(datetime.now().timestamp())}"
        withdrawal_result = self.balance_holder.deduct_balance(currency, amount, transaction_id)
