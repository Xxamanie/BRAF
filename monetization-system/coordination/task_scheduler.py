#!/usr/bin/env python3
"""
Monetization System Task Scheduler
Schedule and manage automation, scraping, and monetization workflows
(Adapted from BRAF/workflows/task_scheduler.py)
"""
import os
import sys
import json
...
# Ensure monetization-system base is on sys.path for sibling package imports
...
class TaskScheduler:
    ...
    def _execute_scraping_task(self, config: Dict) -> Dict:
        """Execute scraping task using monetization-system scrapers"""
        try:
            from scrapers import browser_scraper

            targets = config.get('targets', [])
            if not targets:
                return {'success': False, 'error': 'No targets specified'}

            results = [browser_scraper.run(t) for t in targets]
    ...
    def _execute_automation_task(self, config: Dict) -> Dict:
        """Execute automation task using monetization-system automation"""
        try:
            # Use the production browser automation instance
            from automation.browser_automation import browser_automation

            # Map config to available automation actions
            if 'survey_url' in config:
                survey_data = config.get('answers') or config.get('survey_data') or {}
                return browser_automation.complete_survey_automation(
                    survey_url=config['survey_url'],
                    survey_data=survey_data
                )

            if 'video_url' in config:
                return browser_automation.watch_video_automation(
                    video_url=config['video_url'],
                    watch_duration=config.get('watch_duration')
                )

            return {
                'success': False,
                'error': 'Unsupported automation config. Provide survey_url or video_url.'
            }
    ...
    def _execute_monetization_task(self, config: Dict) -> Dict:
        """Execute monetization task using earnings tracker"""
        try:
            from earnings.earnings_tracker_braf import MonetizationManager
...
def main():
    """Test task scheduler"""
    print("⏰ Testing Task Scheduler (monetization-system)")
    ...
    # Schedule an automation task (demo)
    automation_task = {
        'type': 'automation',
